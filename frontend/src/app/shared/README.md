...
| --- | --- | --- |
| `HeaderComponent` | `app-header` | En-tête global **réellement fixe** (sticky porté par l'hôte) : logo, **méga-menus déroulants par univers** (cartes icône + titre + description, ouverture survol/clavier, fermeture Échap/navigation/clic-dehors), lien Kaikun Pro, CTA connexion, **menu mobile en accordéons**. Aligné sur le prototype client ; chaque lien mène à une page réelle (F2.3 → F2.7). ⚠️ **Connecté, « Mon espace » et « Déconnexion » sont deux boutons distincts côte à côte** (pas un menu replié) : le repli en menu déroulant, posé en F10.5 quand le bouton Assistant vivait aussi dans cette barre, a été annulé le 2026-08-20 après le départ de l'Assistant vers sa bulle flottante dédiée — la barre a de nouveau la place. Le padding horizontal des items de méga-menu a dû être resserré (13px → 10px) pour que « Séjours & Tourisme » et « Kaikun Pro » tiennent sur une ligne avec les deux boutons à côté. |
...
| `PhotoManagerComponent` | `app-photo-manager` | **Bloc « photos » des formulaires de dépôt d'annonce (F8.18).** Choix des fichiers, aperçu avant envoi, retrait, choix de la **couverture**, suppression d'une photo en ligne. Monté par les formulaires de **bien**, de **véhicule** et de **circuit** — cette mécanique n'existait que dans le premier, et c'était **le seul appelant de `POST /media/upload` du frontend** : deux univers sur cinq ne pouvaient être illustrés par aucun moyen. ⚠️ Le **dépôt est différé** (`uploadPending(id)` appelé par le parent après l'enregistrement) : en création l'annonce n'a pas encore d'id. ⚠️ Les photos existantes arrivent par l'**entrée** `initial`, jamais poussées par `viewChild` — le bloc vit dans une branche conditionnelle du gabarit. Entrées : `type` (requis, miroir de `Media::TYPES`), `resourceId`, `initial`, `subject`, `hint`. |
| `AssistantLauncherComponent` | `app-assistant-launcher` | **Le bouton qui ouvre l'assistant**, posé dans les **trois en-têtes** : public (`header`, à côté de « Mon espace »), espaces connectés (`space-header`, avant la cloche) et back-office (`bo-topbar`). ⚠️ **Il n'ouvre rien lui-même** : il bascule `estOuvert` sur le `AssistantStore`, que le tiroir observe — c'est ce qui permet de le poser dans trois chromes différents sans qu'aucun ne monte le tiroir deux fois. Se réduit à une pastille ronde sous 900 px. Masqué, comme le tiroir, quand le serveur a répondu 503. |
| `AssistantPanelComponent` | `app-assistant-panel` | **Assistant Kaikun 360** : le **tiroir** de conversation, qui glisse depuis le bord **droit** vers la gauche et tient toute la hauteur (480 px de large, voile de fond cliquable pour refermer). ⚠️ **La bulle flottante du coin bas-droite a disparu** : elle recouvrait le contenu de chaque page sans qu'on le lui demande et disputait ce coin à `app-scroll-top` et `app-pwa-banner` — le lanceur est maintenant dans les en-têtes (ci-dessus). ⚠️ Le tiroir **ne rend rien tant qu'il est fermé**, donc le même DOM au serveur et au navigateur : l'hydratation tient. Remplace le bouton « Téranga IA » du prototype, qui récitait six phrases en dur — celui-ci interroge le vrai catalogue, la FAQ éditée au back-office et le vrai support. ⚠️ **Monté dans les *layouts*** (`main-layout`, `space-layout`, et `backoffice-layout` avec `variante="back-office"`), **pas dans la racine** : il reste ainsi absent du **parcours d'authentification** (on n'interrompt pas une saisie de mot de passe). ⚠️ **Le composant est l'écran, pas la mémoire** : la conversation vit dans `AssistantStore` (`core/state/`) et survit donc au passage du site public à un espace, qui détruit ce composant — c'est précisément le parcours que l'assistant provoque. Le voile est à 955, le tiroir à 960 — au-dessus de `app-scroll-top` (900) et de `app-pwa-banner` (950), qui ont retrouvé le coin bas-droite pour eux seuls. `Échap` referme, un clic sur le voile aussi, `Entrée` envoie / `Maj+Entrée` passe à la ligne, fil en `aria-live="polite"`. ⚠️ **Les fiches ne sont pas typées, et c'est voulu** : leur forme dépend de l'outil serveur qui répond (annonce du catalogue, entrée de FAQ, dossier personnel depuis F10.2). Le panneau reconnaît les champs qu'il sait afficher — `titre`, `statut`, `lieu`, `periode`, `detail`, `montant`/`prix_xof`, `reference` — et ignore le reste ; un type fermé obligerait à modifier le frontend à chaque outil neuf. ⚠️ `montant` (mis en forme par le serveur) l'emporte sur `prix_xof` (brut) : ne lire que le second laissait réservations, missions et projets **sans leur montant**. ⚠️ **Les résultats défilent en rangée** (carrousel) dès qu'il y en a plusieurs : empilées, cinq annonces occupaient toute la hauteur du fil et poussaient la conversation hors de vue. Le défilement est celui du **navigateur** (`overflow-x` + `scroll-snap`, glissement au doigt, molette inclinée, tabulation) — le TypeScript ne fait que deux choses que CSS ignore : mesurer si quelque chose dépasse (les flèches n'apparaissent qu'alors, et s'effacent en bout de course) et avancer d'une carte au clic. L'écoute du défilement est posée **hors zone Angular** et n'écrit que des classes : un glissement ne déclenche aucun cycle de détection. **Exception : la FAQ reste empilée** — une question et sa réponse se lisent, des cartes de 190 px les rendraient illisibles. |
| `BackLinkComponent` | `app-back-link` | **Bouton « ← Retour » générique.** Revient à la **page précédente** (via l'historique de navigation) plutôt qu'à une cible fixe : posé sur un écran atteint depuis plusieurs endroits (le menu **ou** une notification), il ramène là d'où l'on vient réellement. Repli SSR-safe sur `fallback` (défaut `/mon-espace`) quand l'historique est vide (accès direct, rechargement). ⚠️ **`direct` dès que le libellé NOMME un endroit** (« Mes biens », « Documents ») : un bouton qui promet une destination doit y mener, pas « à la page précédente, quelle qu'elle soit ». Sans lui, la fiche d'un bien atteinte **après une modification** renvoyait au formulaire qu'on venait de quitter, dont le bouton renvoyait à la fiche — on tournait en rond sans revenir à la liste. Entrées : `label` (défaut « Retour »), `fallback`, `direct`. |
| `HideButtonComponent` | `app-hide-button` | **Bouton « Ranger » de la corbeille (F11.5).** Purement présentationnel : il ne connaît ni l'objet rangé, ni la route — chaque écran garde son propre appel (`RequestService.hide`, `BookingService.hide`, `MessageService.hide`, `NotificationService.hide`), parce que ranger reste le geste de l'écran d'origine. Il n'existe que pour que les **quatre** écrans de l'espace client disent la même chose de la même façon. ⚠️ **Ne se rend QUE si `hideable` est vrai**, drapeau décidé par le serveur : une demande en cours, une réservation à venir, un fil non lu ou une notification non ouverte n'ont **aucun bouton**, plutôt qu'un bouton qui échoue en 422. ⚠️ **Le mot est « Ranger », jamais « Supprimer »**, et ce n'est pas de la politesse : rien n'est supprimé. Lire « Supprimer » sur une réservation ferait croire qu'on efface un contrat. ⚠️ **À poser HORS du lien ou du bouton de la carte** : imbriqué, le clic ouvrirait l'élément qu'on fait disparaître, et un `<button>` dans un `<a>` est du HTML invalide. Entrées : `hideable`, `busy` (par élément, jamais un drapeau global qui figerait la liste), `title`. Sortie : `ranger`. |
| `PageHeroComponent` | `app-page-hero` | **Bandeau d'en-tête des grandes pages publiques (F12).** Reprend l'ossature `.uni-hero` que les douze pages recopiaient, et lui ajoute une **image de fond pilotée depuis le back-office**, héritée par les pages filles. ⚠️ **Les textes passés en `input()` restent la vérité par défaut** : la personnalisation back-office ne fait que passer devant quand elle existe — vider un champ au back-office rend à la page son libellé d'origine, il n'existe aucun état « bandeau sans titre ». ⚠️ **L'héritage d'image est résolu côté SERVEUR** (`HeroCatalog`) : le composant lit l'entrée de sa clé, il n'a aucune règle de parenté à connaître — donc aucune à laisser diverger. ⚠️ Le contenu propre à chaque page (points forts, renvois, boutons) est **projeté**, et c'est pour cela que les styles de la variante `--image` vivent dans le style GLOBAL `styles/_universe.scss` : sous encapsulation émulée, une feuille de composant ne touche pas les nœuds projetés. ⚠️ Un fond photo est posé **sous un voile dégradé** + ombre portée sur le texte : le bandeau écrit en blanc, on ne peut pas faire confiance à la photo choisie. Entrées : `heroKey` (requis, doit exister dans `HeroCatalog::BANNERS`), `eyebrow`, `title`, `lead`. |
...
| `ParallaxDirective` | `appParallax` (sur `.k-photo-layer`) | **Léger déplacement vertical d'une couche de fond photo** au défilement (F16.1). Jamais `background-attachment: fixed` (bugué sur iOS Safari) : translate un élément normal, borné à quelques pourcents, actif seulement pendant que la section est visible (`IntersectionObserver`). Posée dans `afterNextRender` (SSR safe), coupée sous `prefers-reduced-motion`. |
