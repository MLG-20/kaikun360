...
| `AssistantPanelComponent` | `app-assistant-panel` | **Assistant Kaikun 360 (F10.1)** : bulle flottante qui déploie une conversation. Remplace le bouton « Téranga IA » du prototype, qui récitait six phrases en dur — celui-ci interroge le vrai catalogue, la FAQ éditée au back-office et le vrai support. ⚠️ **Monté dans les *layouts*** (`main-layout` pour tout le site public, `space-layout` pour les quatre espaces connectés), **pas dans la racine** : il reste ainsi absent du **back-office** (ses outils n'arrivent qu'en F10.3) et du **parcours d'authentification** (on n'interrompt pas une saisie de mot de passe). ⚠️ **Le composant est l'écran, pas la mémoire** : la conversation vit dans `AssistantStore` (`core/state/`) et survit donc au passage du site public à un espace, qui détruit ce composant — c'est précisément le parcours que l'assistant provoque. ⚠️ **Le coin bas-droite est partagé** : la bulle s'empile au-dessus de `app-scroll-top` (z-index 920 contre 900), le panneau déployé passe devant tout (960). SSR-safe (le panneau naît fermé, le DOM concorde), `Échap` referme, `Entrée` envoie / `Maj+Entrée` passe à la ligne, fil en `aria-live="polite"`. ⚠️ **Les fiches ne sont pas typées, et c'est voulu** : leur forme dépend de l'outil serveur qui répond (annonce du catalogue, entrée de FAQ, dossier personnel depuis F10.2). Le panneau reconnaît les champs qu'il sait afficher — `titre`, `statut`, `lieu`, `periode`, `detail`, `montant`/`prix_xof`, `reference` — et ignore le reste ; un type fermé obligerait à modifier le frontend à chaque outil neuf. ⚠️ `montant` (mis en forme par le serveur) l'emporte sur `prix_xof` (brut) : ne lire que le second laissait réservations, missions et projets **sans leur montant**. |
