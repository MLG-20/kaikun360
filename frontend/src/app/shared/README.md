...
| --- | --- | --- |
| `HeaderComponent` | `app-header` | En-tête global **réellement fixe** (sticky porté par l'hôte) : logo, **méga-menus déroulants par univers** (cartes icône + titre + description, ouverture survol/clavier, fermeture Échap/navigation/clic-dehors), lien Kaikun Pro, CTA connexion, bouton **Assistant**, **menu mobile en accordéons**. Aligné sur le prototype client ; chaque lien mène à une page réelle (F2.3 → F2.7). ⚠️ **Connecté, « Mon espace » est un menu replié** qui contient la déconnexion : à deux boutons pleins côte à côte, la barre débordait et la navigation se cassait en deux lignes. Il s'ouvre **au clic** et non au survol, contrairement aux méga-menus — un menu qui contient « Déconnexion » ne doit pas se déplier parce que la souris passait par là — et la sortie y est isolée par un filet. |
